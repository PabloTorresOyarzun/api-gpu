...
from ..services.pipeline.extractor_glm_sdk import procesar_pdf_glm_sdk, ocr_pdf_glm_sdk
...
@post("/procesar-glm-sdk", tags=["Tratamiento"])
async def procesar_endpoint_glm_sdk(
    request: Request,
    data: Annotated[UploadFile, Body(media_type=RequestEncodingType.MULTI_PART)],
) -> dict:
    """
    Pipeline GLM-OCR vía SDK oficial: layout analysis + recognition paralela.

    Diferencia vs /procesar-glm: aquí usamos el SDK `glmocr[selfhosted]` que
    aplica PP-DocLayout-V3 para detectar zonas (texto, tabla, fórmula) y luego
    invoca GLM-OCR con el prompt apropiado por región. El markdown resultante
    está estructurado por layout (no es lectura lineal).

    1. Convierte a PDF (si es Excel o imagen)
    2. Sanitiza (repara, analiza calidad, corrige rotación)
    3. SDK GLM-OCR: layout analysis + recognition por región → markdown
    4. Clasifica páginas por tipo de documento (texto)
    5. Segmenta en documentos individuales
    6. Extrae datos estructurados con Qwen3:14b
    ...
    try:
        ...
        logger.info(f"[procesar-glm-sdk] Paso 0: Convirtiendo {nombre}")
        t0 = time.perf_counter()
        pdf_bytes = await convertir_a_pdf(contenido, nombre)
        tiempos["paso_0_conversion"] = round(time.perf_counter() - t0, 3)

        logger.info(f"[procesar-glm-sdk] Paso 1: Sanitizando ({len(pdf_bytes)} bytes)")
        t0 = time.perf_counter()
        pdf_bytes, alertas = await asyncio.to_thread(sanitizar_pdf, pdf_bytes)
        tiempos["paso_1_sanitizacion"] = round(time.perf_counter() - t0, 3)

        logger.info("[procesar-glm-sdk] Paso 2: SDK GLM-OCR (layout + recognition)")
        t0 = time.perf_counter()
        with tempfile.NamedTemporaryFile(delete=False, suffix=".pdf") as tmp:
            tmp.write(pdf_bytes)
            ruta_temporal = tmp.name
        ...
        try:
            textos_raw = await asyncio.to_thread(ocr_pdf_glm_sdk, ruta_temporal)
        finally:
            if os.path.exists(ruta_temporal):
                os.unlink(ruta_temporal)

        textos_por_pagina = parsear_textos_ocr(textos_raw)
        tiempos["paso_2_ocr_sdk"] = round(time.perf_counter() - t0, 3)

        logger.info(f"[procesar-glm-sdk] Paso 3: Clasificando {len(textos_por_pagina)} páginas")
        t0 = time.perf_counter()
        clasificaciones = clasificar_paginas(textos_por_pagina)
        documentos = await asyncio.to_thread(segmentar_pdf, pdf_bytes, clasificaciones)
        tiempos["paso_3_clasificacion"] = round(time.perf_counter() - t0, 3)

        logger.info(f"[procesar-glm-sdk] Paso 4: Extrayendo datos de {len(documentos)} documento(s)")
        t0 = time.perf_counter()
        resultados = []
        for doc in documentos:
            doc_resultado = {
            ...
            with tempfile.NamedTemporaryFile(delete=False, suffix=".pdf") as tmp:
                ...
                ruta_doc = tmp.name
            try:
                datos = await asyncio.to_thread(procesar_pdf_glm_sdk, ruta_doc, doc["tipo"])
                doc_resultado["datos_extraidos"] = datos
            except TipoDocumentoNoSoportado:
                doc_resultado["datos_extraidos"] = None
            except Exception as e:
                logger.warning(f"Error GLM-SDK extrayendo segmento {doc['tipo']} págs {doc['paginas']}: {e}")
                doc_resultado["datos_extraidos"] = None
                doc_resultado["error_extraccion"] = str(e)
            finally:
                if os.path.exists(ruta_doc):
        ...
        logger.exception(f"Error en pipeline GLM-SDK para {nombre}")
        raise HTTPException(status_code=500, detail=f"Error procesando documento: {str(e)}")
