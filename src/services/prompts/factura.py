...
CHEQUEO DE COHERENCIA (detección de errores, NO sustituto de la lectura): tras transcribir una fila o una sección de totales, verifica mentalmente si los números cuadran entre sí (ej. quantity × unit_price ≈ line_total cuando no hay descuento; suma de líneas ≈ subtotal). Si NO cuadran, NO ajustes el valor con tu cálculo: vuelve a la imagen y RE-LEE el valor sospechoso. El error más típico es perder un dígito de la izquierda en números con separador de miles. La verificación sirve para detectar que leíste mal, no para reemplazar la lectura por una operación.
