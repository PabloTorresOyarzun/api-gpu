...
PROHIBIDO CALCULAR — TRANSCRIPCIÓN LITERAL: TODOS los números (quantity, unit_price, line_total, subtotal, total_amount, descuentos, impuestos) son valores que aparecen IMPRESOS en el documento. Tu trabajo es TRANSCRIBIRLOS exactamente como están escritos, nunca calcularlos, nunca inferirlos, nunca completarlos.
- line_total: léelo de la columna Amount/Total/Importe de cada fila. NO lo calcules como quantity × unit_price.
- unit_price: léelo COMPLETO de la columna Unit Price/Precio Unitario, incluyendo TODOS los dígitos antes de la coma de miles. Si ves "JPY1,244" el valor es 1244 (no 244). Si ves "JPY1,011" el valor es 1011 (no 11 ni 111). Antes de cerrar cada item, MIRA físicamente la celda Unit Price en la imagen: ¿hay un dígito + coma a la izquierda? Si lo hay, INCLÚYELO.
- subtotal, total_amount: léelos del bloque de totales al final del documento. Si el documento dice "To be continued" o es página intermedia y no hay totales impresos, devuelve null. NUNCA sumes los line_total para inventar un total.
- Si un valor no está impreso, devuelve null. JAMÁS inventes un número por cálculo ni por inferencia.
CHEQUEO DE COHERENCIA (solo para detectar errores de lectura, NO para calcular): tras transcribir cada fila, verifica mentalmente si quantity × unit_price ≈ line_total. Si NO cuadra y no hay descuento visible, RE-LEE el valor sospechoso (típicamente unit_price con dígito de miles omitido) directamente de la imagen y corrígelo. La verificación es para detectar errores de lectura, no para reemplazar la lectura.
