...
def ocr_pdf(ruta_pdf: str) -> str:
    ...
    imagenes_todas = convert_from_path(ruta_pdf, dpi=300)

    texto_paginas = []
    
    # Procesar una página a la vez para evitar OOM de GPU en Recognition
    for num_pagina, img in enumerate(imagenes_todas, 1):
        # Preprocesamiento (denoising + contraste adaptativo).
        img_prep = _preprocess_image(img)

        predicciones = _recognition_predictor([img_prep], det_predictor=_detection_predictor)
...
# Límites ajustados para prevenir offloading a CPU. 
# Un modelo 14B con num_ctx=32768 usa ~27GB de VRAM solo en caché KV, colapsando tarjetas de 24GB.
_LLM_LIMITS_DEFAULT = {"num_ctx": 8192, "num_predict": 4096, "timeout": 600}
_LLM_LIMITS_POR_TIPO = {
    "FACTURA_COMERCIAL":    {"num_ctx": 12288, "num_predict": 6144, "timeout": 1800},
    "LISTA_EMBALAJE":       {"num_ctx": 12288, "num_predict": 6144, "timeout": 1800},
    "DOCUMENTO_TRANSPORTE": {"num_ctx": 8192,  "num_predict": 4096, "timeout": 1200},
    "CERTIFICADO_ORIGEN":   {"num_ctx": 8192,  "num_predict": 4096, "timeout": 1200},
}
...
def _llamar_qwen(
    ...
    """
    # Desactivamos el constrained generation (schema) porque en Qwen3 causa un bug
    # en llama.cpp donde se queda atrapado en un bucle infinito repitiendo strings.
    # El esquema se valida posteriormente con Pydantic.
    fmt = "json"
    payload = {
        "model": MODELO,
...
def _validar_con_pydantic(resultado: dict, tipo: str) -> dict:
    ...
    # Desenpaquetar si Qwen envolvió el JSON en una llave root (ej. {"factura": {...}})
    if isinstance(resultado, dict) and len(resultado) == 1:
        primer_valor = next(iter(resultado.values()))
...
def _extraer_documento_chunkeado(texto: str, tipo: str, prompt_sistema: str, limites: dict) -> dict:
    ...
    # PROCESAMIENTO SECUENCIAL: Evita que Ollama intente cargar múltiples 
    # inferencias del modelo 14B en paralelo, lo que agota la VRAM y causa offload a CPU (lentitud extrema).
    for args in args_list:
        idx, resultado = _extraer_chunk(args)
